...
const CashierSideBar = ({ user, isCollapsed }) => {
  
  const navItems = [
    { name: 'Dashboard', path: '/Cashier/CashierDashboard', icon: mdiViewDashboard },
    { name: 'Rental Payment', path: '/Cashier/CashierRentalPayment', icon: mdiCashRegister },
    { name: 'Machine Rates', path: '/Cashier/CashierMachineRate', icon: mdiTableSettings },
  ];
      ...
      <div className={`p-6 flex items-center border-b border-white/10 ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
        ...
        {!isCollapsed && (
          <div className="leading-tight animate-fadeIn">
            ...
            <p className="text-[10px] opacity-80 uppercase">AgriCoop Cashier</p>
          </div>
        )}
      </div>
      ...
      <div className={`p-6 flex items-center border-b border-white/10 ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
        ...
        {!isCollapsed && (
          <div className="animate-fadeIn overflow-hidden">
            <p className="font-bold text-sm truncate">{user?.name || "Cashier Name"}</p>
            <p className="text-xs opacity-70 truncate">{user?.role || "Cashier"}</p>
          </div>
        )}
      </div>
