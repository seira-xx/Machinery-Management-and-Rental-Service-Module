// src/pages/OperationManager/OPMachineRate.jsx
...
const OPMachineRate = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingRate) {
        ...
        // Strip rate_id to let the DB use AUTO_INCREMENT
  ...
  const filteredRates = rates.filter(r => {
    // Convert Numeric ID to string to prevent .toLowerCase() crashes
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {filteredRates.map((rate) => (
              <tr key={rate.rate_id} className="hover:bg-gray-50/50">
            ...
            <div className="p-8">
              ...
              <form onSubmit={handleSave} className="grid grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div className="col-span-2 flex justify-end gap-3 mt-4">
                  ...
                  <button type="submit" className="px-8 py-2 bg-[#10a34a] text-white rounded-lg font-semibold text-sm hover:bg-emerald-700 transition-colors">Save Rate</button>
                </div>
              </form>
            </div>
          </div>
      ...
      {modalState.open && modalState.type === 'success' && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-[1px] flex items-center justify-center z-[60]">
          <div className="bg-white rounded-xl w-[380px] p-10 shadow-2xl text-center flex flex-col items-center animate-scaleIn">
            ...
            <button onClick={() => setModalState({open: false, type: null})} className="w-full py-3 bg-[#10a34a] text-white rounded-xl font-bold hover:bg-emerald-700 shadow-md transition-all">OK</button>
          </div>
        </div>
      )}
    </div>
